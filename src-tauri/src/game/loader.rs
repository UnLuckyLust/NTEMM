...
pub async fn clean_game_mods(path: String) -> Result<LoaderFilesCheck, String> {
...
pub fn check_loader_files_inner(path: &str) -> LoaderFilesCheck {
...
#[tauri::command(rename_all = "camelCase")]
pub async fn check_loader_files(path: String) -> Result<LoaderFilesCheck, String> {
    tauri::async_runtime::spawn_blocking(move || Ok(check_loader_files_inner(&path)))
        .await
        .map_err(|e| e.to_string())?
}
...
pub fn install_loader_files_inner(
    ...
    install_loader_files_inner_with_resource_root(resource_root, path)
}

pub fn uninstall_loader_files_inner(path: String) -> Result<LoaderFilesCheck, String> {
    let loader_dir = loader_target_dir(Path::new(&path));
    let game_check = validate(Path::new(&path));
    ...
    let files = [
        ...
        loader_dir.join("version.dll"),
        loader_dir.join("dsound.dll"),
        loader_dir.join("dinput8.dll"),
    ];
...
pub fn clean_game_mods_inner(path: String) -> Result<LoaderFilesCheck, String> {
    let game_path = Path::new(&path);
    let game_check = validate(game_path);
...
#[tauri::command(rename_all = "camelCase")]
pub async fn install_loader_files(path: String) -> Result<LoaderFilesCheck, String> {
    tauri::async_runtime::spawn_blocking(move || {
        crate::app::elevated::run_loader_files_elevated(
            crate::app::elevated::ElevatedLoaderFilesRequest {
                path,
...
#[tauri::command(rename_all = "camelCase")]
pub async fn uninstall_loader_files(path: String) -> Result<LoaderFilesCheck, String> {
    tauri::async_runtime::spawn_blocking(move || {
        crate::app::elevated::run_loader_files_elevated(
            crate::app::elevated::ElevatedLoaderFilesRequest {
                path,
